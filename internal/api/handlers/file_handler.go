...
// PutThumbnail handles PUT /api/v1/files/{id}/thumbnail
func (h *FileHandler) PutThumbnail(w http.ResponseWriter, r *http.Request) {
	fileID := chi.URLParam(r, "id")
	userID := middleware.GetUserID(r.Context())

	// Thumbnails are small; still cap reasonably.
	r.Body = http.MaxBytesReader(w, r.Body, 8<<20)
	if err := r.ParseMultipartForm(4 << 20); err != nil {
		writeError(w, "invalid thumbnail form", http.StatusBadRequest)
		return
	}
	...
	if s := r.FormValue("original_size"); s != "" {
		originalSize, _ = strconv.ParseInt(s, 10, 64)
	}

	updated, err := h.fileService.PutThumbnail(r.Context(), fileID, userID, iv, mime, originalSize, file)
	if err != nil {
	...
	h.recordUpload(r.Context(), userID, updated.ThumbnailSize)
...
func (h *FileHandler) GetThumbnail(w http.ResponseWriter, r *http.Request) {
	...
	file, getReader, err := h.fileService.GetThumbnail(r.Context(), fileID, userID)
	if err != nil {
		status := http.StatusNotFound
		writeError(w, err.Error(), status)
		return
	...
	if err != nil {
		log.Printf("thumbnail blob missing file_id=%s path=%s: %v", fileID, file.ThumbnailBlobPath, err)
		writeError(w, "thumbnail blob missing", http.StatusNotFound)
		return
	}
	defer reader.Close()

	h.recordDownload(r.Context(), userID, file.ThumbnailSize)

	w.Header().Set("Content-Type", "application/octet-stream")
	w.Header().Set("Content-Length", strconv.FormatInt(file.ThumbnailSize, 10))
	w.Header().Set("X-Thumb-IV", file.ThumbnailIV)
	w.Header().Set("X-Thumb-Mime", file.ThumbnailMime)
	io.Copy(w, reader)
}
