...
- **Client-side decryption** — account unlock plus on-demand per-file keys so encrypted files can be opened on Android (keys stay in memory for the session; they are not bulk-written into AsyncStorage, which is SQLite and was filling with `SQLITE_FULL`). Larger files download to disk and decrypt via native AES-GCM (avoids JS heap OOM)
