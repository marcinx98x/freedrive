...
- **Photos** — Google Photos-style image editor (top icon toolbar, crop/draw/rotate, Adjustments and Filters panels); Save exports JPEG for `.jpg`/`.jpeg`/`.webp` (and JPEG mime types) or PNG otherwise, and renames the file extension to match so Windows/desktop viewers do not mis-decode PNG bytes under a `.jpg` name; **Previous / Next** (toolbar + side chevrons, ←/→ when not typing) switches among images and videos in the current folder listing without leaving the editor (unsaved edits prompt Save / Discard / Cancel)
