...
- **Stream (default):** cloud placeholders online-only until opened; after open/upload they stay available locally (check) until you use **Free up space** (Google Drive–like). In Explorer **Details**, enable the **Status** column (Settings → Explorer integration also explains this). Delete in Explorer soft-trashes on the server (**0.1.57+**). Large folder deletes coalesce to one server folder trash (**0.1.72+**). Offline folder moves (while FreeDrive was closed) relocate the same cloud ID — never create an empty remote folder (**0.1.74+**). After a remote wipe, poll does not recreate empty folder shells from leftover local dirs (**0.1.75+**). Local placeholders are removed only after the remote ID is confirmed gone (**0.1.77+**). Cleanup while FreeDrive is closed is pushed on the next poll (**0.1.71+**). If a multi-file / cloud-only folder delete stalls, upgrade to **0.1.70+**. If Status stays on sync arrows after Free up, upload, Always keep, or open/download, upgrade to **0.1.69+** (In-Sync after hydrate + reconvert demoted placeholders).
