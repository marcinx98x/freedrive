...
- **Stream Status icons (0.1.37+)** — after sync, Explorer shows the cloud / online-only glyph via the **Status** column (Details view → right-click header → enable **Status**). FreeDrive uses CfAPI placeholders (convert + dehydrate + In-Sync), not Dropbox-style shell overlays.
