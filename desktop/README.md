...
- **Stream Status icons (0.1.37+ / 0.1.38+)** — after sync, Explorer shows online-only / available glyphs via the **Status** column (Details → right-click header → **Status** / More). **0.1.38** registers WinRT Storage Provider property definitions and paints icons with `SetAsync` (not Dropbox-style shell overlays). Restart FreeDrive after upgrade so the sync root migrates.
