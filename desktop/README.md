...
- **Background sync** — uploads local changes, polls for remote changes; skips `.git`, `node_modules`, and `.svn` folders during scan; skips Office lock files (`~$…`), `desktop.ini`, `Thumbs.db`, and `*.tmp` in computer sync folders and My Drive; uploads stream-encrypt off disk then use multipart or resumable **8 MiB** chunks when ciphertext **>32 MiB** (Cloudflare-safe); each scan creates/restores remote folders for local subdirectories before uploading files
...
- **Sync root remote trash guard** — version **0.1.34**: if a whole My computer sync folder (e.g. Downloads) is moved to Bin on the web, Desktop stops syncing that folder and asks whether to keep or delete local files — it never silently `remove_dir_all`s the sync root
- **My Drive upload concurrency** — version **0.1.35**: Explorer close uploads share the same 4-wide upload pool as watcher/poll and skip duplicate in-flight paths, so dropping many large files into My Drive no longer encrypts them all at once in RAM
...
- **Drive-like quieter verify (0.1.58+)** — Background verify no longer floods Sync activity with every successful upload (Home progress still updates); legacy CustomState COM server removed; Explorer Settings hint for the Status column.
- **Free up Status cloud (0.1.59+)** — After dehydrate, clear UNPINNED so Explorer Status returns to the cloud glyph instead of stuck sync arrows.
- **Free up real disk reclaim (0.1.60+)** — Dehydrate via CfOpenFileWithOplock + on-disk verify; sweep leftover UNPINNED+hydrated files; clear `hydrate_cache` after folder Free up so AppData plaintext is not left behind.
- **Folder Free up completeness (0.1.61+)** — Coalesce to the highest UNPINNED ancestor; collect the full file list first with N/total progress; final sweep retries every still-hydrated file under the folder (not only UNPINNED).
- **Free up vs FETCH thrash (0.1.62+)** — Mark recent dehydrate before `CfDehydratePlaceholder`; refuse FETCH_DATA under an in-progress Free up tree so Explorer cannot re-download while reclaiming disk.
- **Free up resilience (0.1.63+)** — Per-file timeout + `spawn_blocking` dehydrate; log incomplete Free up ends; auto-resume the same root (max 3) so a stuck CfAPI call cannot silently abandon a multi-thousand-file reclaim.
- **Free up FETCH fail ACK (0.1.64+)** — Cap/align failed `TRANSFER_DATA` Length so huge Free-up skips do not hit `0x80070057` or a false hydrate toast; tiny-file dehydrate no-ops report honest local Status.
- **Free up vs hydrate stall (0.1.65+)** — Refuse all non-PINNED FETCH_DATA while Free up is active (even when path is empty); skip upload-first for files ≥256 MiB without a known content hash so multi‑GB disk images cannot block the walk.
- **Stuck sync arrows after upload (0.1.66+)** — Clear leftover UNPINNED on Stream finalize; poll self-heals hydrated synced files still showing Explorer arrows after Free up on ancestors.
- **Status Syncing flicker (0.1.67+)** — Skip Always-keep hydrate when the file is already local; suppress watcher during pin-copy so Home no longer flickers Up to date ↔ Syncing.
- **Hydrated Status check (0.1.68+)** — Mark In-Sync after FETCH_DATA / hydrate; soft-fail finalize convert `0x8007017C` so Explorer shows check instead of stuck sync arrows.
- **Hydrate reconvert (0.1.69+)** — After `fs::copy` demotes a placeholder, `finalize_hydrated_file` reconverts + In-Sync (fixes Always-keep arrows / `0x80070178`).
- **Cloud-only folder delete (0.1.70+)** — Suppress CLOSE upload while soft-trash is in flight so multi-file Explorer deletes do not stall mid-batch.
- **Offline My Drive cleanup (0.1.71+)** — On poll, push local deletes/moves made while the app was closed (Drive-like) before recreating placeholders from the server.
- **Large folder delete (0.1.72+)** — Coalesce child NOTIFY_DELETE into one folder soft-trash + cap concurrent delete HTTP so huge trees do not stall Explorer.
- **Orphan cloud placeholders (0.1.73)** — Brief purge of unmapped local CfAPI placeholders; superseded by **0.1.74** relink.
- **Safe offline folder move (0.1.74+)** — Cloud-only folder moved while FreeDrive was closed is `PATCH`/DB-relocated via FileIdentity (never empty `create_or_resolve` + soft-trash original). Orphan placeholders are relinked into the DB, not deleted.
- **No empty folder spam (0.1.75+)** — Poll never invents empty remote folders from leftover local dirs after a server wipe (Drive Stream parity). Empty folders sync via watcher/ensure or when the tree has real uploadable files.
- **Desktop shortcut on upgrade (0.1.76+)** — Finish-page “Create desktop shortcut” works even when reinstalling over an existing version (UpdateMode no longer swallows the checkbox).
- **Drive Stream data safety (0.1.77+)** — Poll never removes local placeholders just because a parent listing is empty; probes remote ID first (404 → remove, still exists / network error → keep). Offline soft-trash skips when the probe is uncertain.
- **Empty-file hydrate (0.1.78+)** — Ciphertext that is only the AES-GCM tag (16 B) decrypts to empty plaintext without a size-mismatch error; refresh/mirror failures appear in Sync activity.
- **Sticky Offline clear (0.1.79+)** — After a brief server outage, active My Drive work leaves “Waiting for server…” and returns to Syncing / Up to date instead of staying Offline.
- **Delete/move during Free up (0.1.80+)** — Offline reconcile and soft-trash keep running while Free up walks the tree; delete/rename retry on HTTP 429; Free up backs off under rate limit so user mutations are not starved.
- **Cloud-only move ≠ Delete (0.1.81+)** — Registers `NOTIFY_RENAME` + rename-in-flight so Explorer cloud-only moves are relocated (`PATCH`), not soft-trashed; Delete still soft-trashes after a short grace when identity is gone.
- **Mid-transfer mutations (0.1.82+)** — Rename/delete cancels in-flight uploads and retries at the new path; FETCH_DATA pins via DB path; Free up skips paths under active rename/delete.
...
- **Silent background verify** — on restart, verifies files in the background without a full UI rescan (Home still shows progress; from **0.1.58+** successful uploads are not listed in Sync activity); if initial sync was never completed, startup resumes full sync with a “Resuming sync…” status
...
- **Stream (default):** cloud placeholders online-only until opened; after open/upload they stay available locally (check) until you use **Free up space** (Google Drive–like). In Explorer **Details**, enable the **Status** column (Settings → Explorer integration also explains this). Delete in Explorer soft-trashes on the server (**0.1.57+**). Large folder deletes coalesce to one server folder trash (**0.1.72+**); mass Delete must not leave local leftovers or let poll remirror them (**0.1.83+**). Cloud-file open hydrates on the app runtime so an upload queue cannot leave Explorer spinning (**0.1.84+**). Folder Status follows contents: cloud when files are online-only, check when they are on this PC, not stuck sync arrows (**0.1.85+**). Cloud-only moves use `NOTIFY_RENAME` and relocate the same ID — never soft-trash (**0.1.81+**). Offline folder moves (while FreeDrive was closed) relocate the same cloud ID — never create an empty remote folder (**0.1.74+**). After a remote wipe, poll does not recreate empty folder shells from leftover local dirs (**0.1.75+**). Local placeholders are removed only after the remote ID is confirmed gone (**0.1.77+**). Cleanup while FreeDrive is closed is pushed on the next poll (**0.1.71+**), including during Free up (**0.1.80+**). If a multi-file / cloud-only folder delete stalls, upgrade to **0.1.70+**. If Status stays on sync arrows after Free up, upload, Always keep, or open/download, upgrade to **0.1.69+** (In-Sync after hydrate + reconvert demoted placeholders).
...
- Status glyphs are **native Windows only** (cloud / check / sync). FreeDrive does not register a CustomStateHandler (**0.1.46+**; COM LocalServer removed in **0.1.58+**). If a blank paper icon remains after upgrade: restart FreeDrive, F5 in My Drive, or restart Explorer.
