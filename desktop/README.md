...
- **Stream Status icons (0.1.42+)** — Explorer Status glyphs via WinRT property defs + COM **CustomStateHandler**. FreeDrive must be running. Startup purges stale `FreeDrive!*` SyncRootManager entries and removes the duplicate Explorer **nav pin** that WinRT Register adds alongside our branded NameSpace CLSID (one FreeDrive entry; registry shortcut only — does **not** delete My Drive files). If CfConnect fails with **0x80070186**, Desktop re-registers automatically. If Status is missing in Details: right‑click column headers → **More…** → enable **Status** (`StorageProviderUIStatus` if several appear). Not Dropbox-style shell overlays.
